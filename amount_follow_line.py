...
class PlotAmount(object):
    ...
    def plot_top(self, names, open_price, close_price, max_price, min_price, vol_list,
                 ...
                 save_file="", need_show=False, pct60_list=[], sort_ways='成交额', pct_list=[]):
        """
            绘制蜡烛图
        """
        ...
        pct60_list = [int(i) for i in pct60_list]
        ...
        ax2.set_ylabel('成  \n交  \n额  ', fontweight='bold', fontsize=11, rotation=0)
        ax3.set_ylabel('60 \n日 \n涨 \n幅 ', fontweight='bold', fontsize=11, rotation=0)
        ...
        for i in range(len(pct_list)):
            ...
            y1 = max_price[i] + 0.2
            if y > 0:
                ax1.text(x, y1, str(y), ha='center', va='bottom', fontsize=9, c='black')
            else:
        ...
        ax1.axhline(0, color='b', linestyle='--')
        ...
        for x, y in zip(dates, pct60_list):
            ax3.text(x, y, y, ha='center', va='bottom', fontsize=9)
...
class Utils(object):
    ...
    def judge_good_info(self,name_list, open_rate, close_rate, max_rate, min_rate, vol_list,
        ...
        for i in range(len(name_list)):
            if list(close_rate)[i] < list(ma5_list)[i]:
                continue
            if abs(list(close_rate)[i] - list(max_rate)[i]) >= 1.25:
                continue
            good_list.append(name_list[i])
        print("五日线之上，上影线小于0.25:")
        print("\t".join(good_list))
        return good_list

    # 计算股票MA5信息
    def get_ma5_data(self, df):
        code_list = df['代码'].values.tolist()
        ma5_list = []
        DH = DealHistory()
        for code in code_list:
            mdf = DH.get_eastmoney_trade_data(code=code, period='d', day_num=50)
    ...
    # 时间格式转化为int
    def dt2ts(self, dt, format):
        return int(time.mktime(time.strptime(dt, format)))

    def ts2dt(self, ts, format):
    ...
    def get_time_dif(self):
        ...
        dt = curt[:-5] + "09:30"
        hour = curt[-5:]
        end_dt = self.dt2ts(curt, "%Y-%m-%d %H:%M")
        if hour < "09:30" or hour > "15:00":
            begin_dt = self.dt2ts(dt, "%Y-%m-%d %H:%M")
            dif = 1
        elif hour <= "11:30":
            begin_dt = self.dt2ts(dt, "%Y-%m-%d %H:%M")
            ...
            begin_dt = self.dt2ts(dt, "%Y-%m-%d %H:%M")
            end_dt = self.dt2ts(curt[:-5] + "11:30", "%Y-%m-%d %H:%M")
            dif = (end_dt - begin_dt) / 3600 / 4
        elif hour < "15:00":
            dt = curt[:-5] + "13:00"
            begin_dt = self.dt2ts(dt, "%Y-%m-%d %H:%M")
    ...
    # 计算箱体
    def get_xt_val(self, df, key, val=0, dt="", num=10, is_bottom=True):
        if val != 0:
            return val
        if dt == "-1":
            return 0
...
# 获取实时成交topN只股票信息
    ...
    # 单只股票实时数据
    def crawl_stock_deal_data(self, name, code, params={}, need_plot=False):
        dh = DealHistory()
        util = Utils()
        period = 'd'
        date_len = 5
    ...
    def deal_data_to_df(self, name_list, code_list, need_plot=False):
        data_set = []
        all_infos = ""
        for i in range(len(name_list)):
            stock = name_list[i]
            code = code_list[i]
            content, data_list = self.crawl_stock_deal_data(name=stock, code=code, params={'k_type': 1, 'period': 'd'}, need_plot=need_plot) # 选择成交量还是近60天的涨幅
            all_infos += "{}.{}\n\n".format(i+1, content)
